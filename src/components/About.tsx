        ...
        <div className="relative overflow-hidden rounded-[36px] bg-[#f7f6f3] px-4 pb-0 pt-6 sm:px-8 lg:px-12 lg:pt-8">
          ...
          <div className="relative z-10 grid items-center gap-10 lg:grid-cols-[90px_minmax(0,1fr)]">
            <div className="hidden items-center justify-center lg:flex">
              <div className="flex flex-col gap-6 text-gray-500">
                <a href="https://www.linkedin.com/in/ramesh-edirisinghe/" target="_blank" rel="noreferrer" className="transition hover:text-black">
                  <Linkedin size={18} />
                </a>
                <a href="https://github.com/RameshEdirisinghe" target="_blank" rel="noreferrer" className="transition hover:text-black">
                  <Github size={18} />
                </a>
                <a href="#contact" className="text-xs font-bold uppercase tracking-[0.3em] [writing-mode:vertical-rl]">
                  Contact
                </a>
              </div>
            </div>
            ...
            <div className="grid items-center gap-10 lg:grid-cols-[1fr_360px]">
              <div className="order-2 text-center lg:order-1">
                <p className="mb-3 text-sm font-semibold uppercase tracking-[0.28em] text-[#5f6662]">Software Engineer</p>
              ...
              <div className="order-1 lg:order-2">
                <div className="relative mx-auto w-full max-w-[340px] [mask-image:linear-gradient(to_bottom,black_50%,transparent)] [-webkit-mask-image:linear-gradient(to_top,black_70%,transparent)]">
                  ...
                  <div className="absolute bottom-2 left-1/2 h-10 w-[70%] -translate-x-1/2 rounded-full bg-black/20 blur-xl" />
                  <img
                    src={heroPerson}
                    alt="Ramesh Edirisinghe portrait"
                    className="relative z-10 mx-auto max-h-[520px] object-contain drop-shadow-[0_35px_30px_rgba(0,0,0,0.35)] hero-photo"
                  />
          ...
          <div className="relative z-10 mt-10 sm:mt-14">
            <div className="relative left-1/2 right-1/2 w-screen -translate-x-1/2 bg-[#dfe4e1] py-7 sm:py-8">
              <div className="mx-auto hidden max-w-6xl items-center justify-evenly gap-x-12 gap-y-6 px-6 sm:flex sm:gap-x-16 lg:gap-x-24">
                {partnerLogos.map((logo) => (
                  <div key={logo.alt} className="flex items-center justify-center">
                    <img src={logo.src} alt={logo.alt} className="h-10 w-auto sm:h-12" loading="lazy" />
                  </div>
                ))}
              </div>
              <div className="sm:hidden hero-logo-window">
                <div className="logo-marquee-mobile">
                  {[...partnerLogos, ...partnerLogos].map((logo, idx) => (
                    <div key={`${logo.alt}-${idx}`} className="logo-chip mobile">
                      <img src={logo.src} alt={logo.alt} className="h-9 w-auto" loading="lazy" />
                    </div>
                  ))}
                </div>
              </div>
